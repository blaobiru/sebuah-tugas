...
func middleware(method string, handlerFunc http.HandlerFunc) http.HandlerFunc{
	return func(w http.ResponseWriter, r *http.Request){
		//ngecek apakah method yang direquest sesuai
		if r.Method != method {
			w.WriteHeader(http.StatusMethodNotAllowed)
			return
		}
		//kalau sesuai, langsung ngarahin ke function handler
		handlerFunc(w, r)
	}
}
...
func sendFileHandler(w http.ResponseWriter, r *http.Request){
	...
	fmt.Printf("file yang diterima : %s\n", handler.Filename)
	...
	fmt.Fprintln(w, "sukses terima data")
}
